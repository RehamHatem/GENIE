...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _search() async {
    ...
    List<List<double>> embeddings = imagesSnapshot.docs.map((doc) {
      final data = doc.data() as Map<String, dynamic>;
      return List<double>.from(data['embedding']);
    }).toList();

    final searchEmbedding = await api.generateEmbedding(_searchQuery);
    final response = await api.performSemanticSearch(searchEmbedding, embeddings);

    setState(() {
      _searchResults = response.map((index) => imagesSnapshot.docs[index]).toList();
      _loading = false;
    });
  }
